    ...
    <nav className="sticky top-0 w-full bg-mauve-800 z-50 shadow-md">
      <div className="max-w-6xl mx-auto px-4 h-24 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <form
            className="hidden sm:flex items-center border bg-amber-50 border-amber-500 rounded overflow-hidden"
            onSubmit={hanldeSearch}
          >
            <input
              placeholder="Search here..."
              className="focus:outline-none px-3 py-0.5 w-40"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <button
              type="submit"
              className="px-2 text-gray-500 hover:text-cyan-950 transition duration-200"
            >
              <SearchIcon size={18} />
            </button>
          </form>
      ...
      <div
        className={`md:hidden transition-all duration-300 overflow-hidden ease-in-out ${open ? "max-h-96 opacity-100 translate-y-0" : "max-h-10 opacity-0 translate-y-2"} `}
      >
